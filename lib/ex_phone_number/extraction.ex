      ...
      if not is_nil(metadata) do
        ...
        if String.starts_with?(full_number, country_code_string) do
          ...
          if (not matches_entirely?(metadata.general.national_number_pattern, full_number) and
                matches_entirely?(metadata.general.national_number_pattern, potential_national_number)) or
               test_number_length_against_pattern(metadata.general.possible_number_pattern, full_number) ==
                 ValidationResults.too_long() do
            phone_number =
              if keep_raw_input,
                do: %{
                  phone_number
