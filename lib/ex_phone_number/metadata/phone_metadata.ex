defmodule ExPhoneNumber.Metadata.PhoneMetadata do
    ...
    kwlist =
      ...
      |> xmap(
        ...
        no_international_dialing:
          ~x"./noInternationalDialling"e |> transform_by(&PhoneNumberDescription.from_xpath_node/1),
        available_formats: [
  ...
  defp normalize_boolean(true_char_list) when is_list(true_char_list) and length(true_char_list) == 4,
    do: true
  ...
  def put_default_values(phone_metadata = %PhoneMetadata{}) do
    ...
    phone_metadata = %{
      phone_metadata
      | no_international_dialing:
          process_other_phone_number_description(phone_metadata.no_international_dialing, phone_metadata)
    }

    phone_metadata = %{
      phone_metadata
      | same_mobile_and_fixed_line_pattern:
          phone_metadata.mobile.national_number_pattern == phone_metadata.fixed_line.national_number_pattern
    }
  ...
  def get_national_prefix_formatting_rule(%NumberFormat{national_prefix_formatting_rule: rule}, prefix),
    do: get_national_prefix_formatting_rule(prefix, rule)
  ...
  def get_number_format(%{number_format: number_format}, %PhoneMetadata{} = phone_metadata) do
    ...
    Logger.debug(
      ~s"\tnational_prefix_optional_when_formatting: #{
        ...
        if not is_nil_or_empty?(number_format.domestic_carrier_code_formatting_rule) do
          %{
            domestic_carrier_code_formatting_rule:
              get_domestic_carrier_code_formatting_rule(number_format, phone_metadata.national_prefix)
          }
        else
    ...
    Logger.debug(
      ~s"\tdomestic_carrier_code_formatting_rule: #{
        inspect(number_format.domestic_carrier_code_formatting_rule)
      }"
    )
      ...
      Map.merge(
        ...
        %{
          leading_digits_pattern:
            List.wrap(
              Enum.reduce(number_format.leading_digits_pattern, [], fn %{pattern: pattern}, _ -> pattern end)
            )
        }
      )
  ...
  def process_other_phone_number_description(description, %PhoneMetadata{general: general}) do
    if is_nil(description) do
      %PhoneNumberDescription{
        ...
        possible_number_pattern: Values.description_default_pattern()
      }
    else
